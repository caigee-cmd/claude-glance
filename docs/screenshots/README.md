...
This folder stores notes about the visual assets used by the GitHub README, release notes, and social previews.

这个目录用于存放 GitHub README、Release 页面和社交分享图会用到的素材说明文档。

Screenshot images, demo videos, and exported GIFs in this folder are intentionally kept out of git. The public README uses externally hosted assets, so local source media here should be treated as working files only.

这个目录里的截图、演示视频、导出 GIF 默认不进 git。公开 README 使用的是外链素材，所以这里的本地媒体文件都应视为工作文件，不需要入库。

## Local-Only Assets

Notes:

- `menupannel.png` is the overview screenshot source for the hosted README image.
...
- Use hosted GIF/MP4/image URLs in the README, and avoid committing one-off exports or intermediate renders.
...
- README 首页优先使用外链的 `claude-glance-demo.gif`
- 一个视频入口，直接链接外链的 `claude-glance-demo.mp4`
- 当前版本亮点
- 已知限制
- 安装说明入口
